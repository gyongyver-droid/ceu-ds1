...
The main feature of the regression is that it's linear, while in reality relationship between variables is often not linear. Also, we do not use the transaction date, latitude and logitude variables properly, so these can certainly improved.
...
range(real_estate$latitude)
range(real_estate$longitude)
unique(real_estate$date_correct)


real_estate <- real_estate %>%  mutate(
  lat_from_centre = latitude - lat,
  lon_from_centre = longitude - lon,

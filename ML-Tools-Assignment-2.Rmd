...
We can see, that a valuation accuracy increased from `r round(digits1_history$metrics$val_accuracy[50],4)` to `r round(digits2_history$metrics$val_accuracy[50],4)` in the second model. Besides, we can also see on the training plot, that the valuation accuracy reached this much earlier, it it steeper in the first few epochs.
...
We can see from the accuracy measures, that the softmax activation function performes worse than the relu. So in my last model, I set all of them to relu and also change the optimaser from adam to rmsprop.
